...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['response_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE security_responses SET response_status = ? WHERE id = ?");
    $stmt->bind_param("ii", $newResponseStatus, $responseId);
    $stmt->execute();
    $stmt->close();

    // Redirect to this page to refresh the report table
    header("Location: security-report.php");
    exit();
}
$sql = "SELECT * FROM security_responses WHERE controller = ?";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch item description from security_items table
    $sql2 = "SELECT item_description FROM security_items WHERE id = ?";
