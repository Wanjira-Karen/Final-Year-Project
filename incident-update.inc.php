...
$sql = "SELECT * FROM incident_response WHERE controller = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    while ($item = mysqli_fetch_assoc($result)) {
        ...
        $stmt = $conn->prepare("UPDATE incident_response SET response_status = ? WHERE id = ?");
    ...
    header("Location: ./incident-update.php");
    exit();
}
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch item description from incident_items table
    $sql2 = "SELECT item_description FROM incident_items WHERE id = ?";
