...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['response_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE data_collection_responses SET response_status = ? WHERE id = ?");
    $stmt->bind_param("ii", $newResponseStatus, $responseId);
    $stmt->execute();
    $stmt->close();

    // Redirect to this page to refresh the report table
    header("Location: datacollection-report.php");
    exit();
}
$sql = "SELECT * FROM data_collection_responses WHERE controller = ?";
...
while ($row = $result->fetch_assoc()) {
    ...
    $sql2 = "SELECT item_description FROM data_collection_items WHERE id = ?";
