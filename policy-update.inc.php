...
$sql = "SELECT * FROM policy_responses WHERE controller = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    while ($item = mysqli_fetch_assoc($result)) {
        ...
        $stmt = $conn->prepare("UPDATE policy_responses SET response_status = ? WHERE id = ?");
    ...
    header("Location: ./policy-update.php");
    exit();
}
...
while ($row = $result->fetch_assoc()) {
    ...
    // Fetch item description from policy_items table
    $sql2 = "SELECT item_description FROM policy_items WHERE id = ?";
